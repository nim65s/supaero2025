...
from display_collision_patches import preallocateVisualObjects,updateVisualObjects,COLPATCH_DEFAULT_PREALLOC
...
for t in range(100):
    ...
    if USE_DISTANCE:
        ...
        contact_models = createContactModelsFromDistances(model,data,geom_model,geom_data,10)
    else:
...
assert(len(visual_model.geometryObjects) == len(geom_model.geometryObjects) + 2*COLPATCH_DEFAULT_PREALLOC)
