...
def _createOneConstraint(model,data,geom_model,geom_data,pairId,OC1,OC2,normal,contactId=0):
    '''
...
def createContactModelsFromDistances(model,data,geom_model,geom_data,threshold):
    ...
    for collId,r in enumerate(geom_data.distanceResults):
        if r.min_distance>threshold: continue
        ...
        cm = _createOneConstraint(model,data,geom_model,geom_data,collId,OC1,OC2,r.normal)
        contact_models.append(cm)
