...
for i in range(NINEQ):
    if abs(z[i])>ACCURACY*1e3:
        ...
        assert( (z[i]<0 and abs(Ai[i]@x-bi[i,0])<ACCURACY)
                or  (z[i]>0 and abs(Ai[i]@x-bi[i,1])<ACCURACY) )
...
for i in range(NX):
    if abs(w[i])>ACCURACY*1e3:
        ...
        assert( (w[i]<0 and abs(x[i]-bounds[i,0])<ACCURACY)
                or  (w[i]>0 and abs(x[i]-bounds[i,1])<ACCURACY) )
