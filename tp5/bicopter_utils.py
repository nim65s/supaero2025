...
def plotBicopter(x, f=None, pltAx=None):
    ...
    refs = [
        pltAx.arrow(a + span * c, b + span * s, -s * fr, c * fr, head_width=0.05),
        pltAx.arrow(
            a - span * c, b - span * s, -s * fl, c * fl, head_width=0.05, color="r"
        ),
    ]
    return refs
...
class ViewerBicopter:
    ...
    def __init__(self):
        ...
        self.gname = self.viz.getViewerNodeName(
            self.viz.visual_model.geometryObjects[0], pin.VISUAL
        )
    ...
    def display(self, x):
        ...
        M = pin.SE3(pin.utils.rotate("x", -x[2]), np.array([0, x[0], x[1]]))
